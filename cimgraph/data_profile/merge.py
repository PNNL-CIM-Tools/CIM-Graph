...
This replaces the need for a single hand-maintained "canonical" profile
module (hundreds of dataclasses in one .py) when reading split exports
such as EQ + SSH + TP that together describe one network.

The recommended way to select a merged profile is via the comma-spec
``CIMG_CIM_PROFILE`` environment variable, which ``get_cim_profile()``
resolves automatically::

    export CIMG_CIM_PROFILE="cimgraph.data_profile.cim18gmdm.connectivity,\
        cimgraph.data_profile.cim18gmdm.electrical"

    # Then use any connection and model normally — no cim_override needed:
    from cimgraph.databases import XMLFile
    from cimgraph.models import FeederModel

    file = XMLFile('model.xml')
    network = FeederModel(container=None, connection=file)

For stable, pre-baked merged profiles (such as ``cimhub_2026``), run
``scripts/build_merged_profiles.py`` against ``merge_releases.yaml`` to
generate a committed ``__init__.py`` that is importable directly.

The merged module is profile-agnostic: it works for any set of profile
modules generated by the CIMTool builders, keyed only on the shared
``Identity`` root class.
"""
...
def _first_real_base(cls):
    """Return the first parent of *cls* that isn't ``object``, or ``object``."""
    for base in cls.__mro__[1:]:
...
def _resolve_base(cls, merged_module):
    ...
    return the original.
    """
    base = _first_real_base(cls)
    if base is object:
        return object
    return getattr(merged_module, base.__name__, base)
...
def _build_class(name: str, definitions: list[type],
    ...
    # Every definition of the same class name must agree on its parent.
    # If two profiles declare different bases we cannot pick one without
    # silently dropping a lineage, so fail fast instead.
    base_names = {_first_real_base(cls).__name__ for cls in definitions}
    if len(base_names) > 1:
        raise RuntimeError(
            f'Conflicting base classes for {name!r} across profiles: '
            f'{sorted(base_names)}'
        )
    merged_base = _resolve_base(definitions[0], merged_module)

    # ── 2. Collect the union of direct fields ──────────────────────────
    # Profile fields all carry defaults, so first-seen insertion order is
    # preserved as-is.  A field without a default (should not occur) would
    # raise a normal dataclass TypeError at build time below.
