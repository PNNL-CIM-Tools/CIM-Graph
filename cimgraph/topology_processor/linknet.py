...
class LinkNet():
    ...
    def build_linknet(self, EqTypes):
        ...
        _log.info('Processed ' + str(len(MissingNodes)) + ' missing nodes in ' +
                  str(round(1000 * (time.process_time() - StartTime))) + ' ms')
        ...
        _log.info('Processed ' + str(len(MissingNodes)) + ' missing nodes in ' +
                  str(round(1000 * (time.perf_counter() - StartTime))) + ' ms')
...
# Three-winding transformers not yet supported
    ...
    def build_class_lists(self, eqtype, index, old_counter):
        ...
        for device in self.graph[eqtype].values():
            ...
            _log.info('Processed ' + str(i2 + 1) + ' ' + str(eqtype) + ' objects in ' +
                      str(round(1000 * (time.perf_counter() - StartTime))) + ' ms')
    ...
    def spanning_tree(self, eqtype, RootKeys, Tree, Scope):
        ...
        for i6 in range(len(RootKeys)):
            ...
            if eqtype in [
                    self.cim.Breaker, self.cim.Fuse, self.cim.LoadBreakSwitch, self.cim.Recloser
            ]:
            ...
            # If DER object, only has one node
                    ...
                    self.cim.SynchronousMachine, self.cim.PowerElectronicsConnection,
            ...
            _log.info('Processed topology from  ' + str(root) + ' with ' + str(len(Tree[root])) +
                      ' buses')
