...
class DistributedFeederTopology():
    ...
    def create_distributed_graph(self):
        ...
        sa_index = -1
        boundary_classes = [
            self.cim.Breaker, self.cim.LoadBreakSwitch, self.cim.Recloser, self.cim.Sectionaliser
        ]
            ...
            if cim_class in self.centralized_graph:
                for device in self.centralized_graph[cim_class].values():
                    ...
                    if node1.ConnectivityNodeContainer is None:
                        sa_index = sa_index + 1
                        sa_area_id = list(self.centralized_graph[
                            self.cim.Feeder].keys())[0] + '.' + str(sa_index)
                        sa_uuid = str(uuid.uuid4())
                        sa_container = self.cim.ConnectivityNodeContainer(name=sa_area_id,
                                                                          mRID=sa_uuid)
                        switch_areas.append(
                            SwitchArea(area_id=sa_area_id,
                                       uuid=sa_uuid,
                    ...
                    for node in self.MVTree[device.mRID]:
                        ...
                        GraphModel.add_to_graph(node, switch_areas[sa_index].graph)
                        ProcessedNodes.append(node.mRID)
                        for terminal in node.Terminals:
                            GraphModel.add_to_graph(terminal, switch_areas[sa_index].graph)
                            equipment = terminal.ConductingEquipment
                            GraphModel.add_to_graph(equipment, switch_areas[sa_index].graph)
...
#                                             GraphModel.add_to_graph(lvnode, secondary_area.graph)
#                                             ProcessedNodes.append(lvnode.mRID)
#                                             for lv_terminal in lvnode.Terminals:
#                                                 GraphModel.add_to_graph(lv_terminal, secondary_area.graph)
#                                                 equipment = terminal.ConductingEquipment
#                                                 GraphModel.add_to_graph(equipment, secondary_area.graph)
#                                     switch_areas[sa_index].secondary_areas.append(secondary_area)
        ...
        for node_id in MissingNodes:
            ...
            GraphModel.add_to_graph(node, self.graph)
            for terminal in node.Terminals:
                GraphModel.add_to_graph(terminal, self.graph)
                equipment = terminal.ConductingEquipment
                GraphModel.add_to_graph(equipment, self.graph)
    ...
    def create_switch_areas(self, boundary_classes):
        ...
        EqTypes = [
            self.cim.ACLineSegment, self.cim.RatioTapChanger, self.cim.SynchronousMachine,
        ...
        for cim_class in boundary_classes:
            if cim_class in self.centralized_graph:
                ...
                self.MVTree = self.MVTopology.spanning_tree(cim_class, device_mrids, self.MVTree,
                                                            'all')
        ...
        for i3 in range(len(Xfmrs)):
            ...
            if LVTreeKeys:
                ...
                for i4 in range(len(LVTreeKeys)):
                    ...
                    DistArea['addressable_equipment'].extend(
                        ConnNodeDict[lvnode]['SynchronousMachine'])
                    DistArea['addressable_equipment'].extend(
                        ConnNodeDict[lvnode]['PowerElectronicsConnection'])
                    DistArea['addressable_equipment'].extend(
                        ConnNodeDict[lvnode]['EnergyConsumer'])
                    ...
                    DistArea['addressable_equipment'].extend(
                        ConnNodeDict[lvnode]['PhotovoltaicUnit'])
                    DistArea['unaddressable_equipment'].extend(
                        ConnNodeDict[lvnode]['ACLineSegment'])
                    ...
                    switch_area['unaddressable_equipment'].extend(
                        ConnNodeDict[lvnode]['ACLineSegment'])
                    switch_area['unaddressable_equipment'].extend(
                        ConnNodeDict[lvnode]['Measurement'])
