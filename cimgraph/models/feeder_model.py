...
class FeederModel:
    ...
    def __post_init__(self):
        ...
        if self.distributed:
            self.initialize_distributed_model(self.feeder)
        else:
            self.initialize_centralized_model(self.feeder)
...
#             self.linknet.build_linknet([self.cim.ACLineSegment])
    ...
    def get_all_edges(self, cim_class, graph = {}):
        if graph is None:
            graph = self.graph
        if cim_class in graph:
            self.connection.get_all_edges(self.feeder.mRID, graph, cim_class)
        else:
