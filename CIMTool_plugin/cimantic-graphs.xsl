...
<xsl:stylesheet version="3.0"
    ...
    <xsl:key name="inverse-references" match="a:InverseReference | a:InverseInstance" use="substring-after(@inverseBasePropertyClass, '#')"/>
    ...
    <xsl:template match="a:Catalog">
        <document>
            ...
            <xsl:call-template name="generate-cim-stereotype-enum"/>
    ...
    <xsl:template name="generate-stereotype">
        ...
        <xsl:for-each select="a:Stereotype">
            <item>@stereotype(CIMStereotype.<xsl:value-of select="@label"/>)</item>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template match="a:InverseReference | a:InverseInstance" mode="inverse-association">
    ...
    <xsl:template name="generate-cim-stereotype-enum">
        <xsl:variable name="all-stereotypes" as="xs:string*">
            <xsl:for-each-group select="//a:Stereotype[@label]" group-by="@label">
                <xsl:sort select="@label"/>
                <xsl:sequence select="@label"/>
            </xsl:for-each-group>
        </xsl:variable>
        
        <xsl:if test="exists($all-stereotypes)">
            <item>class CIMStereotype(Enum):</item>
            <list begin="" indent="    " end="">
                <xsl:for-each select="$all-stereotypes">
