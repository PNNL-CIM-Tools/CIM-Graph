...
from cimgraph.databases import GraphDBConnection
...
class TestGraphDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.environ['CIMG_URL'] = 'http://localhost:7200/repositories/powergrid_models'
    ...
    def test_GraphDB_connection_with_env_vars(self):

        connection = GraphDBConnection()
        self.assertIsInstance(connection, GraphDBConnection, 'Connection should be an instance of GraphDBConnection')
        self.assertEqual(connection.cim_profile, 'cimhub_2023', 'CIM profile mismatch')
        self.assertEqual(connection.url, 'http://localhost:7200/repositories/powergrid_models', 'URL mismatch')
